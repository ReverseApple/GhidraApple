...
//        descriptor.addColumn("Receiver Type", true, String::class.java) { it }
        ...
        descriptor.addColumn("Type Result", true, String::class.java) {
            getStaticReceiver(program, it) ?: getAllocedReceiver(program, it) ?: it.typeBoundName
        }
        descriptor.addColumn("Static Receiver", true, String::class.java) { getStaticReceiver(program, it)}
        ...
        descriptor.addColumn("Intra-Procedural Alloc", true, String::class.java) { getAllocedReceiver(program, it) }
        ...
        descriptor.addColumn("Parameter Receiver", true, String::class.java) {
            DataBaseLayer(program).getParamReceiverAtCallsite(it.reference.fromAddress)
        }
...
fun getStaticReceiver(program: Program, it: DynamicDispatchCallsiteData): String? {
    val db = DataBaseLayer(program)
    return db.getStaticReceiverClassAtCallsite(it.reference.fromAddress)?.name
}

fun getAllocedReceiver(program: Program, it: DynamicDispatchCallsiteData): String? {
    val db = DataBaseLayer(program)
    return db.getAllocedReceiverClassAtCallsite(it.reference.fromAddress)?.name
}
